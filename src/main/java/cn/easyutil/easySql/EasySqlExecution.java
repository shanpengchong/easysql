...
import cn.easyutil.easySql.annotations.TableIgnore;
import cn.easyutil.easySql.bean.Page;
import cn.easyutil.easySql.annotations.TableField;
import cn.easyutil.easySql.annotations.TableId;
import cn.easyutil.easySql.annotations.TableName;
import cn.easyutil.easySql.bean.EasySqlResult;
import cn.easyutil.easySql.bean.SQLExecuter;
import cn.easyutil.easySql.config.EasySqlConfiguration;
import cn.easyutil.easySql.config.TransactionUtil;
...
public class EasySqlExecution {
	 ...
	 *
	 * @param flag	数据源标记
	 * @param dataSource	数据源连接池
	 * @param url	数据源连接地址
	 * @param username	数据源连接用户名
	 * @param password	数据源连接用户密码
	 * @return	返回数据源连接池
	 */
...
//			connect.prepareStatement("set names " + charset).executeQuery();
	...
	public  EasySqlResult<Map> queryListPage(String sql, Integer currentPageParam, Integer showCountParam) {
	...
	public <T> EasySqlResult<T> queryListPage(String sql, Class<T> clazz, Integer currentPageParam,
		...
		EasySqlResult<T> easySqlResult = new EasySqlResult<>();
		if (totalResult == 0) {
...
//		TableName annotation = tClass.getDeclaredAnnotation(TableName.class);
		Map<String, TableName> map = getClassAnnotation(tClass, TableName.class);
